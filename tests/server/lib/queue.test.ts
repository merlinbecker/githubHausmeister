...
describe('Queue Management Functions', () => {
  ...
  describe('startNextIfIdle', () => {
    ...
    it('should return early if monthly limit reached', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        'Monthly limit reached for user user-123: 50/50'
      );
      expect(databaseStorage.updateTask).not.toHaveBeenCalled();
    });
  ...
  describe('markTaskCompleted', () => {
    it('should mark task as completed and update monthly counter', async () => {
      ...
      expect(databaseStorage.getUserSystemState).toHaveBeenCalledWith(
        'user-123'
      );
      expect(databaseStorage.updateUserSystemState).toHaveBeenCalledWith(
        'user-123',
        {
          monthlyDone: 6,
        }
    ...
    it('should handle task not found', async () => {
      ...
      expect(databaseStorage.getTaskById).toHaveBeenCalledWith(
        'non-existent-task'
      );
      expect(console.error).toHaveBeenCalledWith(
    ...
    it('should handle errors gracefully', async () => {
      vi.mocked(databaseStorage.getTaskById).mockRejectedValue(
  ...
  describe('markTaskFailed', () => {
    it('should mark task as failed with reason', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        'Task task-1 marked as failed: GitHub API error'
      );
    });
    ...
    it('should mark task as failed without reason', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        'Task task-1 marked as failed: Unknown error'
      );
    });
