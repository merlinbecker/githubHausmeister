...
export default function RepositoryManager({
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description:
          'Repository has been added to monitoring and webhooks have been configured.',
      });
  ...
  const removeMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description:
          'Repository has been removed from monitoring and webhooks have been deleted.',
      });
      onRefresh();
    },
    onError: (error: Error) => {
  ...
  const handleRemoveRepository = (repositoryId: string) => {
    if (
      window.confirm(
        'Are you sure you want to remove this repository from monitoring? This will also delete the webhook.'
      )
    ) {
    ...
    githubRepos?.filter(
      (repo) =>
        !userRepositories.some(
          (ur) => ur.owner === repo.owner.login && ur.repo === repo.name
        )
    ) || [];
        ...
        {userRepositories.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-github-muted">
              Add repositories to start automated maintenance tasks.
            </p>
          </div>
        ) : (
      ...
      {showAddRepo && (
        <div className="border-t border-github-border pt-6">
          ...
          {loadingRepos ? (
            ...
            <div className="text-center py-4">
              <p className="text-github-muted">
                No more repositories available or all repositories are already
                added.
              </p>
            </div>
          ) : (
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2 text-xs text-github-muted">
                        {repo.permissions?.admin && (
                          <span className="bg-github-green/20 text-github-green px-2 py-1 rounded-full">
                            Admin
                          </span>
                        )}
