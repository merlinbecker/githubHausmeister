...
export default function TaskQueue({ queue, onRefresh }: TaskQueueProps) {
  ...
  const getTypeLabel = (labels: string[]) => {
    return labels.find(label => ["tests", "lint", "types", "security", "docs"].includes(label)) || "chore";
  };
  ...
  return (
    <section className="bg-github-surface border border-github-border rounded-lg p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-sm text-github-muted" data-testid="text-queue-count">
          {queue.length} tasks
        </span>
      </div>
      ...
      {queue.length === 0 ? (
        <div className="text-center py-8">
          ...
          <h3 className="text-lg font-medium text-github-text mb-2">Queue is Empty</h3>
          <p className="text-github-muted">Add new tasks using the form below to get started.</p>
        </div>
      ) : (
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <div className="flex items-center space-x-2 mb-1">
                    <span className="font-medium text-github-text" data-testid={`text-repo-${task.id}`}>
                      {task.owner}/{task.repo}
                    </span>
                    <span className={`px-2 py-1 text-xs rounded-full ${getTypeColor(task.labels)}`}>
                      {getTypeLabel(task.labels)}
                    </span>
                  </div>
                  <h3 className="text-sm text-github-text" data-testid={`text-title-${task.id}`}>
                    {task.title}
                  </h3>
                </div>
              </div>
