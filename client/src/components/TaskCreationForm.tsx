...
export default function TaskCreationForm({ onRefresh }: TaskCreationFormProps) {
  ...
  return (
    <section className="bg-github-surface border border-github-border rounded-lg p-6">
      <h2 className="text-lg font-semibold text-github-text mb-4">Create New Tasks</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <Label htmlFor="taskCount" className="block text-sm font-medium text-github-text mb-2">
              Number of Tasks
            </Label>
            <Input
              id="taskCount"
          ...
          <div className="space-y-2">
            {templates.map(template => (
              <label key={template.id} className="flex items-center space-x-3">
                ...
                />
                <span className="text-sm text-github-text">{template.label}</span>
