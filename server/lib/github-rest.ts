...
export async function listAllCollaborators(
  ...
  console.log(
    `📋 [COLLABORATORS] Listing all collaborators for ${owner}/${repo}:`
  );
  ...
  try {
    ...
    console.log(
      `📋 [COLLABORATORS] Found ${collaborators.length} collaborators:`
    );
    ...
    collaborators.forEach((collaborator, index) => {
      ...
      console.log(
        `📋 [COLLABORATORS] ${index + 1}. ${type} ${collaborator.login} (ID: ${collaborator.id}) - Permissions: ${permissions}`
      );
      ...
      if (
        ...
        console.log(
          `   🔍 [COLLABORATORS] *** POTENTIAL COPILOT BOT FOUND: ${collaborator.login} ***`
        );
      }
    });
  } catch (error: any) {
    console.log(
      `❌ [COLLABORATORS] Error listing collaborators: ${error.status} ${error.message}`
    );
  }
}
...
export async function checkCopilotAvailability(
  ...
  console.log(
    `🔍 [COPILOT CHECK] Starting availability check for repository ${owner}/${repo}`
  );
  ...
  for (const username of copilotUsernames) {
    ...
    try {
      ...
      console.log(
        `🔍 [COPILOT CHECK] Checking if ${username} is a collaborator on ${owner}/${repo}...`
      );
      ...
      console.log(
        `🔍 [COPILOT CHECK] Collaborator check response status: ${response.status}`
      );

      if (response.status === 204) {
        console.log(
          `✅ [COPILOT CHECK] SUCCESS: Found assignable Copilot agent: ${username}`
        );
        return { available: true, username };
      }
    } catch (error: any) {
      console.log(
        `❌ [COPILOT CHECK] Collaborator check failed for ${username}: ${error.status} ${error.message}`
      );

      // Check if user exists at all
      try {
        console.log(
          `🔍 [COPILOT CHECK] Checking if user ${username} exists globally...`
        );
        const userResponse = await octokit.rest.users.getByUsername({
          username,
        });
        console.log(
          `✅ [COPILOT CHECK] User ${username} exists globally (ID: ${userResponse.data.id}) but is not a collaborator on ${owner}/${repo}`
        );
      } catch (userError: any) {
        console.log(
          `❌ [COPILOT CHECK] User ${username} does not exist globally: ${userError.status} ${userError.message}`
        );
      }
    }
  }
  ...
  console.log(
    `❌ [COPILOT CHECK] FAILURE: No Copilot agents found available for ${owner}/${repo}`
  );
...
export async function assignCopilotToIssue(
  ...
  repo: string,
  issueNumber: number
): Promise<{ success: boolean; assignedAgent?: string; error?: string }> {
  const octokit = new Octokit({ auth: token });

  console.log(
    `🎯 [COPILOT ASSIGN] Starting assignment process for issue #${issueNumber} in ${owner}/${repo}`
  );
  ...
  if (!copilotCheck.available) {
    const errorMsg =
      'Copilot agent not available for this repository. Repository may not have Copilot enabled or agent not configured.';
  ...
  console.log(
    `✅ [COPILOT ASSIGN] Step 1 SUCCESS: Found available Copilot agent: ${copilotCheck.username}`
  );

  try {
    console.log(
      `🎯 [COPILOT ASSIGN] Step 2: Assigning ${copilotCheck.username} to issue #${issueNumber}...`
    );
    ...
    console.log(
      `🎯 [COPILOT ASSIGN] Assignment API response status: ${response.status}`
    );
    console.log(
      `🎯 [COPILOT ASSIGN] Response assignees count: ${response.data.assignees?.length || 0}`
    );

    // Verify assignment was successful
    const assignees = response.data.assignees || [];
    console.log(
      `🎯 [COPILOT ASSIGN] Current assignees: ${assignees.map((a) => a.login).join(', ')}`
    );

    const copilotAssigned = assignees.some(
      (assignee) => assignee.login === copilotCheck.username
    );

    if (copilotAssigned) {
      console.log(
        `✅ [COPILOT ASSIGN] SUCCESS: ${copilotCheck.username} is now assigned to issue #${issueNumber}`
      );
      ...
      console.log(
        `❌ [COPILOT ASSIGN] Expected: ${copilotCheck.username}, Got: [${assignees.map((a) => a.login).join(', ')}]`
      );
      return {
        success: false,
        error: errorMsg,
    ...
    console.error(
      `❌ [COPILOT ASSIGN] Assignment API ERROR: ${error.status} ${error.message}`
    );
...
export async function verifyCopilotAssignment(
  ...
  repo: string,
  issueNumber: number
): Promise<{ isAssigned: boolean; assignedCopilot?: string }> {
  const octokit = new Octokit({ auth: token });

  console.log(
    `🔍 [COPILOT VERIFY] Starting verification for issue #${issueNumber} in ${owner}/${repo}`
  );
  ...
  try {
    ...
    console.log(
      `🔍 [COPILOT VERIFY] Retrieved issue data, assignees count: ${issue.assignees?.length || 0}`
    );
    ...
    console.log(
      `🔍 [COPILOT VERIFY] Current assignees: [${assignees.map((a) => a.login).join(', ')}]`
    );
    console.log(
      `🔍 [COPILOT VERIFY] Looking for Copilot usernames: [${copilotUsernames.join(', ')}]`
    );

    for (const assignee of assignees) {
      if (copilotUsernames.includes(assignee.login)) {
        console.log(
          `✅ [COPILOT VERIFY] SUCCESS: Found Copilot agent ${assignee.login} assigned to issue #${issueNumber}`
        );
    ...
    console.log(
      `❌ [COPILOT VERIFY] FAILURE: No Copilot agents found in assignees for issue #${issueNumber}`
    );
    return { isAssigned: false };
  } catch (error: any) {
    console.error(
      `❌ [COPILOT VERIFY] API ERROR: ${error.status} ${error.message}`
    );
...
export async function findSimilarOpenIssues(
  token: string,
  owner: string,
  repo: string,
  title: string,
  ...
  const queries = [
    ...
    keywords.length > 0
      ? `repo:${owner}/${repo} type:issue state:open ${keywords.join(' ')}`
      : null,
    // Search by labels if provided
    labels.length > 0
      ? `repo:${owner}/${repo} type:issue state:open label:"${labels[0]}"`
      : null,
  ].filter(Boolean);
...
export async function checkForDuplicateIssue(
  ...
  const similarIssues = await findSimilarOpenIssues(
    token,
    owner,
    repo,
    title,
  ...
  const exactMatch = similarIssues.find(
    (issue) => issue.title.toLowerCase() === titleLower
  );
